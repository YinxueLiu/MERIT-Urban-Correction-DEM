...
#2021/01/19
#This script can be used to construct the Random Forest model in a combined cities mode (70% sample train, 30% sample test).
...
readcity <- function(cityname,offset){
  ...
  path2 <- paste0('path of regression datasets',cityname,'/RF_layers/') #path of regression factor layers
...
#load('7city_Berlin city_non forest.RData')
#######################################
############################################################
#pack the last city estimation as a function
regression <- function(city,traincity,pathre,ds_com,ds,merit,ma_f){
  # estimate one city with other five cities' data
  set.seed(1)
  #use the same train data to predict the third city
  ind <- sample(2,nrow(ds_com),replace=TRUE,prob=c(0.7,0.3))
  traindata <- ds_com[ind==1,c(1:15)]
  #traindata <- traindata[traindata$diff>0,1:15]
  testdata <- ds[,c(1:15)]#######################################################
  testdatatarget <- ds[,16:17]################################################
  testdataxy <- ds[,17:18]####################################################
  ...
  p <- ggplot(data=dfim, aes(x=reorder(Factors,Importance),y=Importance)) +
    ...
    ggtitle(paste0(city,'*'),subtitle=traincity) +#################
  geom_text(aes(label=percent(dfim$Importance,digits = 0)),nudge_y=0.04,size=3.5)+
  ...
  p_new <-p_new+ylab('Predict error(m)')+xlab('Actual error(m)')
  ...
  ds[,16] <- MERIT_ori- pred######
  ds_1 <- ds#ds_2 estimated error was applied to MERIT
  ds[indexout,16] <- MERIT_ori[indexout]#restore the outlier to original MERIT data
  ds_2 <- ds#ds_2 estimated error was not applied to MERIT
  MERIT_new <- ds[,16]############
  ...
  path2 <- paste0('/home/qo18712/RF_server/',city,'/city/')#######################################################
  ...
  merit[ma_f] <- ds_1[,16]#here the MERIT was updated by applying predicted error to all pixels ###########
  MERITra[] <- resize(merit,nrow(MERIT)*ncol(MERIT),1)
  writeRaster(MERITra,filename=paste0(pathre,paste0(traincity,'-',city,'_MERIT_',varname,'_worldpop_update.tif'))######
              , format="GTiff",overwrite=TRUE)
  merit[ma_f] <- ds_2[,16]#here the MERIT was updated by applying predicted error to all pixels but outliers###########
  MERITra[] <- resize(merit,nrow(MERIT)*ncol(MERIT),1)
...
############################################ final output
pathname <- 'resultpath'
############################################ Carlisle by Manchester Bristol Cambridge
city <- city7
traincity <- paste(city1,city2,city6,sep=' ')
pathre <- paste0(pathname,
                 traincity,'-',city,'/')
if (!dir.exists(pathre)){dir.create(pathre)}
ds_com <- rbind(ds1,ds2,ds6)
ds <- ds7
regression(city,traincity,pathre,ds_com,ds,merit7,ma_f7)



##############################################without Carlisle included in combined model,5-1
city <- city4 ############################################# Beijing
traincity <- paste(city5,city3,city1,city2,city6,sep=' ')
pathre <- paste0(pathname,
                 traincity,'-',city,'/')
if (!dir.exists(pathre)){dir.create(pathre)}
ds_com <- rbind(ds5,ds3,ds1,ds2,ds6)
ds <- ds4
regression(city,traincity,pathre,ds_com,ds,merit4,ma_f4)
############################################# 5-1 Berlin no forest
city <- city5
traincity <- paste(city4,city3,city1,city2,city6,sep=' ')
pathre <- paste0(pathname,
                 traincity,'-',city,'/')
if (!dir.exists(pathre)){dir.create(pathre)}
ds_com <- rbind(ds4,ds3,ds1,ds2,ds6)
ds <- ds5
regression(city,traincity,pathre,ds_com,ds,merit5,ma_f5)
############################################# London
city <- city3
traincity <- paste(city4,city5,city1,city2,city6,sep=' ')
pathre <- paste0(pathname,
                 traincity,'-',city,'/')
if (!dir.exists(pathre)){dir.create(pathre)}
ds_com <- rbind(ds4,ds5,ds1,ds2,ds6)
ds <- ds3
regression(city,traincity,pathre,ds_com,ds,merit3,ma_f3)

############################################# Manchester
city <- city1
traincity <- paste(city4,city5,city3,city2,city6,sep=' ')
pathre <- paste0(pathname,
                 traincity,'-',city,'/')
if (!dir.exists(pathre)){dir.create(pathre)}
ds_com <- rbind(ds4,ds5,ds3,ds2,ds6)
ds <- ds1
regression(city,traincity,pathre,ds_com,ds,merit1,ma_f1)
############################################# Bristol
city <- city2
traincity <- paste(city4,city5,city3,city1,city6,sep=' ')
pathre <- paste0(pathname,
                 traincity,'-',city,'/')
if (!dir.exists(pathre)){dir.create(pathre)}
ds_com <- rbind(ds4,ds5,ds3,ds1,ds6)
ds <- ds2
regression(city,traincity,pathre,ds_com,ds,merit2,ma_f2)
############################################################ Cambridge
city <- city6
traincity <- paste(city4,city5,city3,city1,city2,sep=' ')
pathre <- paste0(pathname,
                 traincity,'-',city,'/')
if (!dir.exists(pathre)){dir.create(pathre)}
ds_com <- rbind(ds4,ds5,ds3,ds1,ds2)
ds <- ds6
regression(city,traincity,pathre,ds_com,ds,merit6,ma_f6)